...
class MyDBCLI:
    ...
    def run(self):
        print("Welcome to MyDB NoSQL CLI!")
        while True:
            user_input = input("MyNoSQL > ").strip()
            ...
            if tokens[0] in CRUDSet:
                ...
                if command == "create" and tokens[1] == "table":
                    ...
                    if table_name in self.mydb.table_names:
                        ...
                        print("Current Table:", self.mydb.current_table)
                        ...
                        for i, d in enumerate(data):
                            ...
                            if col != self.mydb.current_table[table_name]['columns'][i]:
                                print("col:", col)
                                print("val:", self.mydb.current_table[table_name]['columns'][i])
                ...
                if command == "load":
                    ...
                    if ".json" in file_path:
                        file_path = json_to_csv(file_path, file_path.replace('json', 'csv'))
                ...
                elif command == "view" and tokens[1] == "table":
                    # Example: view table person
                    table_name = tokens[2]
                    self.mydb.load_table(table_name, self.directory)
                    self.mydb.show_table(table_name)

                elif command == "view" and tokens[1] == "tables":
                    # Example: view tables
                    ...
                    # It will start FROM "IF" and be seperated by "AND" or the end of the list
                if command == "SHOW":
                    output_col_name = tokens[tokens.index("SHOW") + 1:tokens.index("IN")]
                    table_name = tokens[tokens.index("IN") + 1]
                    ...
                    if "COMBINE" in tokens:
                        end_index = tokens.index("ORDER") if "ORDER" in tokens else len(tokens)
                        joins = tokens[
                                tokens.index("COMBINE") + 1: end_index]

                    if "ORDER" in tokens and tokens.index("ORDER") != len(tokens) - 1:
                        # SHOW everything IN shop IF age > 60 AND price > 1000 COMBINE shopping BY customer_id EQUAL customer_id in descending ORDER OF age
                    ...
                    if joins:
                        ...
                        if order_type:
                            ...
                            path = os.path.join(current_directory, "myDB", "Database", self.directory, "tables",
                                                table_name)
                            # path = f"/Users/Aven/Desktop/myDB_NoSQL/Database/{self.directory}/tables/{table_name}"
